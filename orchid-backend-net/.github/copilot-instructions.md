...
- Method stages represent experiment log progression, while sample stages represent biological growth progression on each sample. Workflow design should keep these stage systems distinct and coordinated rather than conflated. Prefer gating sample stage progression by biological evidence rather than strict method-stage time windows, because plant growth can be slower than planned method duration. Eligibility for sample-stage promotion requires an approved monitoring report for the sample, stage requirements within threshold, and the sample not being infected; method-stage overdue must be warning-only and never block stage progression.
