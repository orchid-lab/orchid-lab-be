...
namespace orchid_backend_net.Application.User.UpdateUser
{
    public class UpdateUserInformationCommandValidator : AbstractValidator<UpdateUserInformationCommand>
    {
        ...
        private void Configure()
        {
            ...
            RuleFor(x => x)
                .MustAsync(async (request, cancellationToken) => await BeUniqueEmail(request.Id, request.Email,cancellationToken))
                .When(u => !string.IsNullOrEmpty(u.Email))
            ...
            RuleFor(x => x)
                .MustAsync(async (request, cancellationToken) => await BeUniquePhoneNumber(request.Id, request.PhoneNumber, cancellationToken))
                .When(u => !string.IsNullOrEmpty(u.PhoneNumber))
                .WithMessage("Số điện thoại đã tồn tại.");
        }

        private async Task<bool> BeUniqueEmail(string userId, string? email, CancellationToken cancellationToken)
        {
            ...
            return existedUser.ID == userId;
        }

        private async Task<bool> BeUniquePhoneNumber(string userId, string? phoneNumber, CancellationToken cancellationToken)
        {
